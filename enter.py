...
def search(driver,city,isdebug,web_order):
    ...
    for j in range(url_num):#对每个公司的链接，进入该网址，获取信息
        ...
        except Exception as e:#“电话：暂无联系方式” 这种形势虽然有
        ...
        try:
            # phone = driver.find_element_by_xpath("/html/body/div[3]/div[4]/ul/li[2]/span/font/a/p").text
            ...
            phone = driver.find_element_by_xpath("/html/body/div[4]/div[1]/div[1]/a[2]/p").text
            print("********************************"+str(phone)+"****************************")
            try:
                d = phone.split(';')[1]
            except:
        ...
        try:#第1次获取
            ...
            print('5'+str(Exception)+' '+str(e)+' '+repr(e)+' ')
            print('order:'+str(j))
            print(url)
            workbook.save(city+'_'+web_order+'.xls')
            return
...
if __name__=='__main__':
    ...
    if(isdebug==0):#默认不显示浏览器图形界面
        ...
        driver = webdriver.Chrome(chrome_options=chrome_options,executable_path="D:\\Code\\Python\\crawler\\chromedriver.exe")
